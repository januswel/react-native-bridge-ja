...
As mentioned above, every module will have it's own [GCD](https://developer.apple.com/library/ios/documentation/General/Conceptual/ConcurrencyProgrammingGuide/OperationQueues/OperationQueues.html) Queue by default, unless it specifies the queue it wants to run on, by implementing the `-methodQueue` method or synthesizing the `methodQueue` property with a valid queue. The exceptions are `View Managers`[^4] (That extend `RCTViewManager`) that will use the `Shadow Queue` by default, and the special target `RCTJSThread`, which is just a placeholder, since it's a `thread` rather than a `queue`.

[^4]: View Managers not a real exception, since the base class explicitly specifies the shadow queue as the target queue.
